...
public class Draft_code extends Draft {
    ...
    public Draft_code(IExtension inputExtension) {
        this(Collections.singletonList(inputExtension));
    }

    public Draft_code(List<IExtension> inputExtensions) {
        this(inputExtensions, Collections.singletonList(new Protocol("")));
    }

    public Draft_code(List<IExtension> inputExtensions, List<IProtocol> inputProtocols) {
        this(inputExtensions, inputProtocols, 2147483647);
    }

    public Draft_code(List<IExtension> inputExtensions, int inputMaxFrameSize) {
        this(inputExtensions, Collections.singletonList(new Protocol("")), inputMaxFrameSize);
    }

    public Draft_code(List<IExtension> inputExtensions, List<IProtocol> inputProtocols, int inputMaxFrameSize) {
        this.log = LoggerFactory.getLogger(Draft_code.class);
...
//        }
    ...
    public Draft copyInstance() {
        ...
        return new Draft_code(newExtensions, newProtocols, this.maxFrameSize);
    }
    ...
    private Framedata translateSingleFrame(ByteBuffer buffer) throws IncompleteException, InvalidDataException {
        if (buffer == null) {
            ...
            if (payloadlength < 0 || payloadlength > 125) {
                Draft_code.TranslatedPayloadMetaData payloadData = this.translateSingleFramePayloadLength(buffer, optcode, payloadlength, maxpacketsize, realpacketsize);
...
//            frame.isValid();
    ...
    private Draft_code.TranslatedPayloadMetaData translateSingleFramePayloadLength(ByteBuffer buffer, Opcode optcode, int oldPayloadlength, int maxpacketsize, int oldRealpacketsize) throws InvalidFrameException, IncompleteException, LimitExceededException {
        if (optcode != Opcode.PING && optcode != Opcode.PONG && optcode != Opcode.CLOSING) {
            ...
            return new Draft_code.TranslatedPayloadMetaData(payloadlength, realpacketsize);
        } else {
